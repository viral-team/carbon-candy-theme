...
function getWorkspaceConfiguration() {
	...
	for (const key of configKeys) {
		...
		const valueGroup = vscode.workspace.getConfiguration("carbon_candy").inspect(PKG_PROP_MAP[key]);
...
function updateConfig(config) {
	...
	for (const key in config) {
		log.info(`carbon_candy.${key} changed, updating to ${config[key]}`);
		const updateHandler = updateThemeJSONHandlers[key];
		if (updateHandler) {
			vscode.workspace.getConfiguration("carbon_candy").update(key, config[key], true);
